...
export const handleDeleteContent = async (
  ...
  try {
    const res = await axios.delete(BASE_URL + "/content/delete?id=" + queryId);
    const data = res.data;
    if (data > 0) {
      dispatch(
        openSnackbar({
...
export const handlePostContent = async (
  ...
  dispatch(setLoading(true));
  try {
    const res = await axios.post(BASE_URL + "/content", formData);
    const data = res.data;
...
export const handleUpdateContent = async (
  ...
  dispatch(setLoading(true));
  try {
    const res = await axios.put(BASE_URL + "/content", formData);
    const data = res.data;
